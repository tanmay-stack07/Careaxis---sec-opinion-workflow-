...
FALLBACK_DOCTOR_POOL = [
    {"name": "Dr. Ananya Rao", "specialty": "Internal Medicine"},
    {"name": "Dr. Karan Mehta", "specialty": "Pulmonology"},
    {"name": "Dr. Priya Nair", "specialty": "Cardiology"},
    {"name": "Dr. Rohan Patel", "specialty": "Infectious Disease"},
    {"name": "Dr. Neha Sharma", "specialty": "Endocrinology"},
]
...
def build_analysis_prompt(payload: Dict[str, Any], history: List[Dict[str, Any]]) -> str:
    ...
    return (
        ...
        "The payload includes the doctor's diagnosis. Compare that diagnosis against standard clinical guidelines,\n"
        "and estimate how much it deviates from standard guidance.\n\n"
        f"Current Case:\n{current_case}\n\n"
        ...
        "- deviation_percentage: number between 0 and 100 where 0 means fully aligned with guidelines\n"
        "- suggested_doctors: array with 2-3 objects. Each object must include:\n"
        "  - name: string\n"
        "  - specialty: string\n"
        "  - reason: string (why this specialist is suitable)"
    )
...
def _validate_analysis_output(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        deviation_percentage = float(data["deviation_percentage"])
    except (TypeError, ValueError) as exc:
        raise ValueError("deviation_percentage must be a number") from exc

    if deviation_percentage < 0 or deviation_percentage > 100:
        raise ValueError("deviation_percentage must be between 0 and 100")

    suggested_doctors_raw = data["suggested_doctors"]
    if not isinstance(suggested_doctors_raw, list):
        raise ValueError("suggested_doctors must be a list")

    suggested_doctors: List[Dict[str, str]] = []
    for item in suggested_doctors_raw:
        ...
        suggested_doctors.append(
            {
                ...
                "reason": reason or "Suggested based on current triage findings.",
            }
        )

    if len(suggested_doctors) < 2:
        needed = 2 - len(suggested_doctors)
        fallback = random.sample(FALLBACK_DOCTOR_POOL, k=min(needed, len(FALLBACK_DOCTOR_POOL)))
        for doctor in fallback:
            suggested_doctors.append(
                {
                    "name": doctor["name"],
                    "specialty": doctor["specialty"],
                    "reason": "Fallback specialist suggestion due to incomplete model output.",
                }
            )

    suggested_doctors = suggested_doctors[:3]
